...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, FormFieldComponent, RouterLink],
...
export class CrFormComponent implements OnInit {
  ...
  async retryProjects(): Promise<void> {
    await this.projects.loadAll();
  }
  ...
  async submit(): Promise<void> {
    ...
    try {
      ...
      if (this.selectedFile) {
        try {
          await this.details.create(created.id, this.ticketComment.trim(), this.selectedFile);
        } catch (error) {
          notice += ` The attachment was not saved: ${apiErrorMessage(error, 'upload failed')}`;
        }
      }
      await this.router.navigate(['/change-requests', created.id], { state: { notice } });
    } catch (error) {
      this.error.set(apiErrorMessage(error, 'We could not submit this change request.'));
    } finally {
